...
func main() {
	...
	for _, filePath := range config.Files {
		asset := &Assets{
			VarName: safeVarName(strings.TrimSuffix(filepath.Base(filePath), filepath.Ext(filePath)), knownVars),
			Path:    filePath,
			Value: func(path string) []byte {
				f, err := os.ReadFile(path)
				if err != nil {
					panic(fmt.Sprintf("Error reading filePath %s: %v", path, err))
				}
				if len(f) == 0 {
					return []byte{}
				}
				return []byte(strings.TrimRight(string(f), "\n"))
			},
		}
...
// This filePath was generated by goxorstring tool for package {{ .PackageName }}.
package {{ .PackageName }}
...
var (
{{- range $name, $x := .Xors }}
    {{ $name }} = &goxorstring.XorString{
    Key: {{ $x.Key }},
    Encrypted: []byte{
    {{- range $i, $b := $x.Encrypted }}{{ if $i }}, {{ end }}{{ $b }}{{ end }}},
    }
{{- end }}
)
`

	t := template.Must(template.New("xor").Parse(tmpl))
	...
	if config.Output != "" {
		// Ensure the output filePath exists (create or truncate)
		f, err := os.OpenFile(config.Output, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
		if err != nil {
			panic(fmt.Sprintf("Error opening output filePath %s: %v", config.Output, err))
		}
		defer f.Close()

		// Write the formatted code to the filePath
		_, err = f.Write(formatted)
		if err != nil {
			panic(fmt.Sprintf("Error writing to output filePath %s: %v", config.Output, err))
		}
...
func safeVarName(name string, knownVars map[string]int) string {
	...
	name = cases.Title(language.BrazilianPortuguese).String(name)
	inBytes = []byte(name)
